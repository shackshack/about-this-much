...
export default function DaySnapshot({ dateStr, logs, profile, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "flex-end", zIndex: 55 }} onClick={onClose}>
      <div style={{ background: "white", borderRadius: "20px 20px 0 0", padding: "1.25rem", width: "100%", maxHeight: "85vh", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
        ...
        <p style={{ fontSize: "12px", color: "var(--text-muted)", marginBottom: "12px" }}>Past days are locked — this is a look back, not editable.</p>
        ...
        {logs.length === 0 ? (
          ...
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "12px", marginBottom: "1rem" }}>
              <MetricRing label="Fiber" value={sum("fiber_g")} target={profile.fiber_target_g} unit="g" baseColor="#FFC300" icon="🌾" direction="goal" />
              <MetricRing label="Protein" value={sum("protein_g")} target={profile.protein_target_g || 46} unit="g" baseColor="#6A0DAD" icon="🍗" direction="goal" />
              <MetricRing label="Water" value={sum("water_oz")} target={profile.water_target_oz} unit="oz" baseColor="#00CFFF" icon="💧" direction="goal" />
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "12px", marginBottom: "1rem" }}>
              <MetricRing label="Sugar" value={sum("sugar_g")} target={profile.sugar_target_g} unit="g" baseColor="#FF4F81" icon="🍬" direction="limit" />
